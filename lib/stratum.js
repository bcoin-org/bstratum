...
Connection.prototype.increaseBan = function increaseBan(score) {
  ...
  if (this.banScore >= Stratum.BAN_SCORE) {
    this.logger.debug(
      'Stratum: ban score exceeds threshold %d (%s).',
...
Job.prototype.fromTemplate = function fromTemplate(attempt) {
  ...
  this.difficulty = common.getDifficulty(this.target);
  return this;
};
...
Job.prototype.hash = function hash(nonce1, nonce2, ts, nonce) {
  var hash = this.attempt.hash(nonce1, nonce2, ts, nonce);
  return new Share(hash, nonce1, nonce2, ts, nonce);
};
...
Job.prototype.toJSON = function toJSON() {
  return [
    this.id,
    common.hswap32(this.attempt.prevBlock),
...
function Share(hash, nonce1, nonce2, ts, nonce) {
  this.hash = hash;
  this.nonce1 = nonce1;
  this.nonce2 = nonce2;
  this.ts = ts;
  this.nonce = nonce;
  this.difficulty = common.getDifficulty(hash);
}
